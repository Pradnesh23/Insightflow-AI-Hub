...
export function CorrelationAnalysis({ datasetId, data, columns }: CorrelationAnalysisProps) {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setCorrelations(result.result_data)
    } catch (error) {
                  ...
                  {Object.entries(correlations).map(([row, values]) => (
                    <tr key={row} className="border-b border-surface-light/20">
                      ...
                      {Object.entries(values).map(([col, value]) => {
                        ...
                        const hue = (value as number) > 0 ? 120 : 0
                        return (
                          <td
                            key={`${row}-${col}`}
                ...
                {Object.entries(correlations)
                  .flatMap(([col1, values]) =>
                    Object.entries(values)
                      .filter(([col2, value]) => col1 < col2 && Math.abs(value as number) > 0.7)
                      .map(([col2, value]) => ({
                        col1,
                        col2,
