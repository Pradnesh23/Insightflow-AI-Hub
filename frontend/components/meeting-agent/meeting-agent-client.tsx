...
export function MeetingAgentClient({ userId }: MeetingAgentClientProps) {
  ...
  const exportToPDF = () => {
    ...
    doc.text(topic, pageWidth / 2, yPos, { align: "center" })
    yPos += 15
    ...
    doc.setFontSize(10)
    doc.setFont("helvetica", "normal")
    const agendaLines = doc.splitTextToSize(result.agenda || "No agenda", pageWidth - 40)
    ...
    doc.setFontSize(10)
    doc.setFont("helvetica", "normal")
    const researchLines = doc.splitTextToSize(result.research || "No research", pageWidth - 40)
    ...
    doc.save(`meeting-${topic.replace(/\s+/g, "-").toLowerCase()}.pdf`)
  }
  ...
  const exportToDOCX = async () => {
    ...
    saveAs(blob, `meeting-${topic.replace(/\s+/g, "-").toLowerCase()}.docx`)
  }
  ...
  const exportToJSON = () => {
    ...
    a.href = url
    a.download = `meeting-${topic.replace(/\s+/g, "-").toLowerCase()}.json`
            ...
            {result ? (
              <>
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <Button onClick={exportToDOCX} variant="outline" size="sm" className="gap-2">
                    <FileText className="w-4 h-4" />
                    DOCX
                  </Button>
                </div>
