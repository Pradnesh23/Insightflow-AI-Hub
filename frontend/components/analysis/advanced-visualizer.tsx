...
import { CustomChartRequest } from "./custom-chart-request"
...
export function AdvancedVisualizer({ data, columns }: AdvancedVisualizerProps) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {topVisualizations.map((rec, idx) => (
              <Card
                key={idx}
                className={`glass-effect border-surface-light/20 cursor-pointer transition-all ${selectedVisualization?.type === rec.type && selectedVisualization?.title === rec.title
                  ? "ring-2 ring-primary"
      ...
      {/* AI Custom Chart Request */}
      <CustomChartRequest data={data} columns={columns} />
    </div>
  )
}
